...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="premium-card overflow-hidden">
            <div class="px-6 py-5 border-b border-white/5 bg-white/3">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-sm font-black uppercase tracking-widest text-white">Tambah Developer</h2>
                </div>
            </div>
            <div class="p-6">
                @unless($isTrash)
                    <form action="{{ route('admin.developers.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 mb-2">Nama Developer</label>
                            <input type="text" name="name" value="{{ old('name') }}" required
                                   class="admin-input" placeholder="misal: Valve Corporation">
                        </div>
                        <button type="submit"
        ...
        <div class="lg:col-span-2 flex flex-col gap-4">
            ...
            <div class="flex items-center gap-3 flex-wrap lg:flex-nowrap">
                ...
                <form method="GET" action="{{ route('admin.developers.index') }}" class="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <div class="relative flex-1 min-w-0">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari developer..."
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.developers.index', $isTrash ? ['trash'=>1] : []) }}"
                           class="flex-shrink-0 flex items-center gap-1.5 px-3 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest
                                  text-gray-500 hover:text-red-400 border border-transparent hover:border-red-500/30 transition-all">
            ...
            <div class="premium-card overflow-hidden">
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="border-b border-white/5 bg-white/3">
                                <th class="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Nama Developer</th>
                                <th class="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 text-center w-32">Jumlah Game</th>
                        ...
                        <tbody>
                            @forelse($developers as $developer)
                                <tr class="data-row">
                                    <td class="px-6 py-4">
                                        <span class="text-sm font-bold text-white">{{ $developer->name }}</span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            @if($isTrash)
                                                <form action="{{ route('admin.developers.restore', $developer->developer_id) }}" method="POST" class="inline"
                                                      onsubmit="return adminConfirmSubmit(event, 'Apakah Anda yakin ingin mengembalikan developer {{ addslashes($developer->name) }}?', 'info', 'Restore Developer');">
                                                    @csrf
                                                    <button type="submit" class="px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest
                                                                                  bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all">
                                                        Restore
                                                    </button>
                                                </form>
                                                <form action="{{ route('admin.developers.force-destroy', $developer->developer_id) }}" method="POST" class="inline"
                                                      onsubmit="return adminConfirmSubmit(event, 'Apakah Anda yakin ingin menghapus permanen developer ini? Data tidak dapat dikembalikan.', 'danger', 'Hapus Permanen');">
                                                    @csrf @method('DELETE')
                                                    <button type="submit" class="px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest
                                            ...
                                            @else
                                                <button onclick="openEditModal({{ $developer->developer_id }}, '{{ addslashes($developer->name) }}')"
                                                ...
                                                <form action="{{ route('admin.developers.destroy', $developer->developer_id) }}" method="POST" class="inline"
                                                      onsubmit="return adminConfirmSubmit(event, 'Yakin ingin menghapus developer {{ addslashes($developer->name) }}?', 'danger', 'Hapus Developer');">
                                                    @csrf @method('DELETE')
                                                    <button type="submit" class="px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest
                                                                                  bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20 transition-all">
                                                        Hapus
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="px-6 py-12 text-center text-sm font-bold text-gray-500">
                                        {{ $isTrash ? 'Tidak ada developer terhapus.' : 'Belum ada developer.' }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <div class="px-6 py-4 border-t border-white/5 bg-white/2 flex items-center justify-between gap-4">
                    <p class="text-xs font-bold text-gray-500">Total: <span class="text-gray-300">{{ $developers->total() }}</span> developer</p>
                    {{ $developers->links() }}
                </div>
            </div>
        </div>
    ...
    <div id="edit-modal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 hidden items-center justify-center">
        <div class="premium-card w-full max-w-md p-6 mx-4">
            <h3 class="text-base font-black uppercase tracking-widest text-white mb-5">Edit Developer</h3>
