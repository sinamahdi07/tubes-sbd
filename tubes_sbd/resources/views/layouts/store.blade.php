...
<body class="min-h-screen text-white bg-[#050a12] antialiased">
    ...
    <nav class="fixed bottom-6 left-4 right-4 z-[9999] flex h-24 items-center justify-around rounded-[2.5rem] border-2 border-white/10 bg-[#07111d]/95 pb-safe px-3 shadow-[0_25px_60px_rgba(0,0,0,0.8)] backdrop-blur-3xl lg:hidden">
        ...
        <a href="{{ route('games.search') }}" class="mobile-nav-item flex flex-1 flex-col items-center justify-center gap-1.5 {{ request()->routeIs('games.search') ? 'is-active' : 'text-gray-400' }}">
            ...
            <span class="text-[11px] font-black uppercase tracking-wider">Search</span>
        ...
        <a href="{{ url('/wishlist') }}" class="mobile-nav-item relative flex flex-1 flex-col items-center justify-center gap-1.5 {{ request()->is('wishlist*') ? 'is-active' : 'text-gray-400' }}">
            <div class="relative">
                ...
                @if($wishlistCount > 0)
                    <span class="absolute -right-2 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-[#3bd2ff] text-[9px] font-black text-[#07111d] ring-2 ring-[#07111d]">
                        {{ $wishlistCount }}
                    </span>
                @endif
            </div>
            <span class="text-[11px] font-black uppercase tracking-wider">Wishlist</span>
            @if(request()->is('wishlist*')) <div class="active-indicator"></div> @endif
        </a>

        {{-- Store Center --}}
        <a href="{{ route('home') }}" class="mobile-nav-item flex flex-1 flex-col items-center justify-center -mt-14">
            <div class="flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-tr from-[#118dff] to-[#66c0f4] text-white shadow-[0_15px_35px_rgba(17,141,255,0.5)] border-8 border-[#050a12] active:scale-95 transition-transform">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                </svg>
            </div>
            <span class="text-[12px] font-black uppercase tracking-widest text-[#66c0f4] mt-2">Store</span>
        </a>
        ...
        <a href="{{ url('/chat') }}" class="mobile-nav-item flex flex-1 flex-col items-center justify-center gap-1.5 {{ request()->is('chat*') ? 'is-active' : 'text-gray-400' }}">
            ...
            <span class="text-[11px] font-black uppercase tracking-wider">Chat</span>
        ...
        <a href="{{ route('profile.show') }}" class="mobile-nav-item flex flex-1 flex-col items-center justify-center gap-1.5 {{ request()->routeIs('profile.show') ? 'is-active' : 'text-gray-400' }}">
            <div class="h-10 w-10 overflow-hidden rounded-full border-2 {{ request()->routeIs('profile.show') ? 'border-[#66c0f4]' : 'border-gray-500' }}">
            ...
            <span class="text-[11px] font-black uppercase tracking-wider">Me</span>
