...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="premium-card overflow-hidden">
            <div class="px-6 py-5 border-b border-white/5 bg-white/3">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-sm font-black uppercase tracking-widest text-white">Tambah Genre</h2>
                </div>
            </div>
            <div class="p-6">
                @unless($isTrash)
                    <form action="{{ route('admin.genres.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 mb-2">Nama Genre</label>
                            <input type="text" name="name" value="{{ old('name') }}" required
                                   class="admin-input" placeholder="misal: Action, RPG, Strategy...">
                        </div>
                        <button type="submit"
        ...
        <div class="lg:col-span-2 flex flex-col gap-4">
            ...
            <div class="flex items-center gap-3 flex-wrap lg:flex-nowrap">
                ...
                <form method="GET" action="{{ route('admin.genres.index') }}" class="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <div class="relative flex-1 min-w-0">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari genre..."
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.genres.index', $isTrash ? ['trash'=>1] : []) }}"
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="border-b border-white/5 bg-white/3">
                                <th class="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Nama Genre</th>
                        ...
                        <tbody>
                            @forelse($genres as $genre)
                                <tr class="data-row">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span class="text-sm font-black text-gray-300">{{ $genre->games_count }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            @if($isTrash)
                                                <form action="{{ route('admin.genres.restore', $genre->genre_id) }}" method="POST" class="inline"
                                                      onsubmit="return adminConfirmSubmit(event, 'Apakah Anda yakin ingin mengembalikan genre {{ addslashes($genre->name) }}?', 'info', 'Restore Genre');">
                                                    @csrf
                                                ...
                                                <form action="{{ route('admin.genres.force-destroy', $genre->genre_id) }}" method="POST" class="inline"
                                                      onsubmit="return adminConfirmSubmit(event, 'Apakah Anda yakin ingin menghapus permanen genre ini? Data tidak dapat dikembalikan.', 'danger', 'Hapus Permanen');">
                                                    @csrf @method('DELETE')
                                            ...
                                            @else
                                                ...
                                                <form action="{{ route('admin.genres.destroy', $genre->genre_id) }}" method="POST" class="inline"
                                                      onsubmit="return adminConfirmSubmit(event, 'Yakin ingin menghapus genre {{ addslashes($genre->name) }}?', 'danger', 'Hapus Genre');">
                                                    @csrf @method('DELETE')
                            ...
                            @empty
                                <tr><td colspan="3" class="px-6 py-12 text-center text-sm font-bold text-gray-500">{{ $isTrash ? 'Tidak ada genre terhapus.' : 'Belum ada genre.' }}</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                <div class="px-6 py-4 border-t border-white/5 bg-white/2 flex items-center justify-between gap-4">
                    <p class="text-xs font-bold text-gray-500">Total: <span class="text-gray-300">{{ $genres->total() }}</span> genre</p>
                    {{ $genres->links() }}
                </div>
            </div>
        </div>
    ...
    <div id="edit-modal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 hidden items-center justify-center">
        <div class="premium-card w-full max-w-md p-6 mx-4">
            <h3 class="text-base font-black uppercase tracking-widest text-white mb-5">Edit Genre</h3>
