...
@section('content')
        ...
        <div class="premium-card overflow-hidden">
            <div class="px-6 py-5 border-b border-white/5 bg-white/3">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-sm font-black uppercase tracking-widest text-white">Tambah Publisher</h2>
                </div>
            </div>
            <div class="p-6">
                @unless($isTrash)
                    <form action="{{ route('admin.publishers.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 mb-2">Nama Publisher</label>
                            <input type="text" name="name" value="{{ old('name') }}" required
                                   class="admin-input" placeholder="misal: Electronic Arts">
                        </div>
                        <button type="submit"
        ...
        <div class="lg:col-span-2 flex flex-col gap-4">
            ...
            <div class="flex items-center gap-3 flex-wrap lg:flex-nowrap">
                <div class="flex items-center gap-1 p-1 rounded-xl bg-white/5 border border-white/8 flex-shrink-0">
                    <a href="{{ route('admin.publishers.index', request()->except(['trash', 'page'])) }}"
                    ...
                    <a href="{{ route('admin.publishers.index', array_merge(request()->except('page'), ['trash' => 1])) }}"
                       class="px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap
                              {{ $isTrash ? 'bg-red-500/80 text-white shadow-lg shadow-red-500/20' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                        Terhapus
                    </a>
                </div>
                <form method="GET" action="{{ route('admin.publishers.index') }}" class="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <div class="relative flex-1 min-w-0">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari publisher..."
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.publishers.index', $isTrash ? ['trash'=>1] : []) }}"
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="border-b border-white/5 bg-white/3">
                                <th class="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Nama Publisher</th>
                        ...
                        <tbody>
                            @forelse($publishers as $publisher)
                                <tr class="data-row">
                                    <td class="px-6 py-4">
                                        <span class="text-sm font-bold text-white">{{ $publisher->name }}</span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            @if($isTrash)
                                                <form action="{{ route('admin.publishers.restore', $publisher->publisher_id) }}" method="POST" class="inline"
                                                      onsubmit="return adminConfirmSubmit(event, 'Apakah Anda yakin ingin mengembalikan publisher {{ addslashes($publisher->name) }}?', 'info', 'Restore Publisher');">
                                                    @csrf
                                                ...
                                                <form action="{{ route('admin.publishers.force-destroy', $publisher->publisher_id) }}" method="POST" class="inline"
                                                      onsubmit="return adminConfirmSubmit(event, 'Apakah Anda yakin ingin menghapus permanen publisher ini? Data tidak dapat dikembalikan.', 'danger', 'Hapus Permanen');">
                                                    @csrf @method('DELETE')
                                            ...
                                            @else
                                                ...
                                                <form action="{{ route('admin.publishers.destroy', $publisher->publisher_id) }}" method="POST" class="inline"
                                                      onsubmit="return adminConfirmSubmit(event, 'Yakin ingin menghapus publisher {{ addslashes($publisher->name) }}?', 'danger', 'Hapus Publisher');">
                                                    @csrf @method('DELETE')
                            ...
                            @empty
                                <tr><td colspan="3" class="px-6 py-12 text-center text-sm font-bold text-gray-500">{{ $isTrash ? 'Tidak ada publisher terhapus.' : 'Belum ada publisher.' }}</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                <div class="px-6 py-4 border-t border-white/5 bg-white/2 flex items-center justify-between gap-4">
                    <p class="text-xs font-bold text-gray-500">Total: <span class="text-gray-300">{{ $publishers->total() }}</span> publisher</p>
                    {{ $publishers->links() }}
                </div>
            </div>
        </div>
    ...
    <div id="edit-modal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 hidden items-center justify-center">
        <div class="premium-card w-full max-w-md p-6 mx-4">
            <h3 class="text-base font-black uppercase tracking-widest text-white mb-5">Edit Publisher</h3>
